...
func makeRestAPIPayloadHttpRequestRewriteRules(
...
func resourceCudaWAFHttpRequestRewriteRulesCreate(d *schema.ResourceData, m interface{}) error {
	resourceEndpoint := baseURI + "/services/" + d.Get("parent.0").(string) + "/http-request-rewrite-rules"
	resourceCreateResponseError := makeRestAPIPayloadHttpRequestRewriteRules(
		d,
		"POST",
		resourceEndpoint,
	)
...
func resourceCudaWAFHttpRequestRewriteRulesUpdate(d *schema.ResourceData, m interface{}) error {
	resourceEndpoint := baseURI + "/services/" + d.Get("parent.0").(string) + "/http-request-rewrite-rules/" + d.Get("name").(string)
	resourceUpdateResponseError := makeRestAPIPayloadHttpRequestRewriteRules(
		d,
		"PUT",
		resourceEndpoint,
	)
...
func resourceCudaWAFHttpRequestRewriteRulesDelete(d *schema.ResourceData, m interface{}) error {
	resourceEndpoint := baseURI + "/services/" + d.Get("parent.0").(string) + "/http-request-rewrite-rules/" + d.Get("name").(string)
	resourceDeleteResponseError := makeRestAPIPayloadHttpRequestRewriteRules(
		d,
		"DELETE",
		resourceEndpoint,
	)
